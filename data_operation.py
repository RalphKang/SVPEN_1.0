...
class normalization_data_store:
    def __init__(self, norm_dir4state, norm_dir4observation, feasible4state, observation, state_size, error_zoomer):
        """
        ...
        """
        self.state_norm = np.loadtxt(norm_dir4state)
        self.orig_observation = observation
        orig_obse_norm = np.loadtxt(norm_dir4observation)
        self.obse_norm, self.pad_need = self.check_stder_dimension(orig_obse_norm)
        self.feasible_domain = feasible4state
        self.state_size = state_size
        self.error_zoomer = error_zoomer

    def check_stder_dimension(self, spec_stder):
        """
        this function is used to check whether the observation normalziaiton neeeds to be padded,
        as the spectrum dimension can be changed according to the setting,
        more details can check Arxiv: 2210.06071, expanding band
        therefore, the normalization needs to change also to normalize expanded spectrum
        ---------------------------------
        INPUT:
        spec_stder: the normalization values for spectrum
        -------------------------------------------
        OUTPUT:
        spec_stder: the padded normalization values
        pad_need: bool value, used to notify whether padding has been done.
        """
        if self.orig_observation.shape[-1] != spec_stder.shape[-1]:
            padding_np = ((0, 0), (0, self.orig_observation.shape[-1] - spec_stder.shape[-1]))
            spec_stder = np.pad(spec_stder, padding_np, 'mean')
            pad_need = True
            print("dimension is larger than the default one, padding is needed")
        else:
            pad_need = False
        return spec_stder, pad_need

    def obse_trans_phy2state_est(self, observation):
        """
        used to normalize spectrum obtained from radis to the spectrum format needed for state estimator
        ---------------------------------------------------------
        INPUT:
        ...
        """
        data_norm = (observation - self.obse_norm[1]) / (self.obse_norm[0] - self.obse_norm[1])
        data4net = torch.from_numpy(np.expand_dims(np.expand_dims(data_norm, axis=0), axis=0)).float()
        return data4net

    def state_unnorm(self, normed_state):
        """
        transform normalized state to unnormalized ones by the original normalization boundaries
        ------------------------------------
        INPUT:
        self: used the state normalization values inside
        normed_state: the normalized state (only feed)
        OUTPUT:
        un-normalized state
        """
        return normed_state * (self.state_norm[0] - self.state_norm[1]) + self.state_norm[1]

    def state_normalization(self, est_state):
        """
        ...
        """
        return (est_state - self.state_norm[1]) / (self.state_norm[0] - self.state_norm[1])
...
class ReplayBuffer:
    ...
    def sample(self, n, seed=5, seed_needed=False):
        ...
        for transition in mini_batch:
            observation, state, error = transition
...
def sample_from_buffer_v3(i, history_buffer, in_situ_memory, global_search_memory,
                          current_norm_state, current_observation, current_error, batch_size, partition=4):
    """
    ...
    history_buffer: the buffer stores the data from the guess of the network
    ...
    partition: type: int; the maximum amount of samples from history and in-situ buffer, the default value of which
    ...
    """
    divide_ratio = int(batch_size / partition)
    if i <= divide_ratio:
        observation_history, state_history, error_history = history_buffer.sample(i)
        observation_insitu, state_insitu, error_insitu = in_situ_memory.sample(i)
        observation_search, state_search, error_search = global_search_memory.sample(batch_size - i * 2)
    else:  # fetch half from each one
        observation_history, state_history, error_history = history_buffer.sample(divide_ratio)
        observation_insitu, state_insitu, error_insitu = in_situ_memory.sample(divide_ratio)
        observation_search, state_search, error_search = global_search_memory.sample(
            batch_size - divide_ratio * 2)
        # print("")
    observation_set = torch.cat((observation_history, observation_insitu, observation_search), dim=0)
    state_set = torch.cat((state_history, state_insitu, state_search), dim=0)
    # error_set = torch.squeeze(torch.cat((error_history, error_insitu, error_search), dim=0))
    error_set = torch.cat((error_history, error_insitu, error_search), dim=0)
...
def sample_from_buffer_v4(i, in_situ_memory, global_search_memory, bayes_memory,
                          current_norm_state, current_observation, current_error, batch_size, partition=4):
    """
    this function is used to sample and stack data from several buffers, in order to provide samples for network to use.
    Compared to V3, V4 includes a new buffer: bayes memory, which contains the historical memory of bayes optimization**
    -------------------------------------------------------------
    INPUT:
    i: i is the epoch, which is the indicator of the sample ratio from each buffer
    in-situ_memory: the buffer stores the noised historical data
    global_search_memory: the buffer stores the random search data
    bayes_memory: the buffer stores the bayes optimization data
    current_norm_state, current_observation, current_error: the current prediction of the network
    batch_size: type: int; how many samples needed to construct a buffer
    partition: type: int; the maximum amount of samples from history and in-situ buffer, the default value of which
                is 1/4 of batch size
    ...
    if i <= sample_limit:
        observation_insitu, state_insitu, error_insitu = in_situ_memory.sample(i)
        observation_bayes, state_bayes, error_bayes = bayes_memory.sample(i)
        observation_search, state_search, error_search = global_search_memory.sample(batch_size - i * 2)
    else:  # fetch half from each one
        observation_insitu, state_insitu, error_insitu = in_situ_memory.sample(sample_limit)
        observation_bayes, state_bayes, error_bayes = bayes_memory.sample(sample_limit)
        observation_search, state_search, error_search = global_search_memory.sample(
            batch_size - sample_limit * 2)
        # print("")
    observation_set = torch.cat((observation_insitu, observation_bayes, observation_search), dim=0)
    state_set = torch.cat((state_insitu, state_bayes, state_search), dim=0)
    error_set = torch.cat((error_insitu, error_bayes, error_search), dim=0)
    # # add the guide from current action
    current_state = torch.tensor(current_norm_state, dtype=torch.float).view(1, 1, -1)
    current_observation = torch.tensor(current_observation, dtype=torch.float).view(1, -1)
    current_error = torch.tensor(current_error, dtype=torch.float).view(1, -1)
