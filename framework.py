...
"""
SVPEN1.1 :
This version is reorganized old V1.0, in order to compare with new version2.10
"""
# config
...
parser.add_argument('--prestore_dir', type=str, default="buffer_storage/global_search_buffer.pkl", help='prestore dir')
parser.add_argument('--error_threshold', type=float, default=0.15, help='error threshold')
...
global_search_step = 100  # the number of initial global search steps

optimization_recording = pandas_recording(column=args.state_meaning + ['error'],
                                          save_dir='output/best_state_recording.csv')
...
forward_physical_model = forward_model_radis(molecule="CO2", waveneed=wave_need,
                                             lightpath=10,
                                             databank='hitemp', spec_type='absorptivity')  # initialize physical model
...
if error_first > args.error_threshold:
    print("Inverse function mode does not work well, state estimation:{},error:{}".format(state_est_first, error_first))
    ...
    gradient_recording = pandas_recording(
        ...
        save_dir='output/network_recording.csv')
    # %%
